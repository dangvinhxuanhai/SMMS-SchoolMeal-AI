...
# Số record tối thiểu để cho phép train (dùng cho test).
# Nếu không set biến môi trường ML_MIN_TRAIN_SAMPLES thì mặc định = 0 (không kiểm tra).
MIN_TRAIN_SAMPLES = int(os.getenv("ML_MIN_TRAIN_SAMPLES", "0"))
...
def get_first_school_id_from_logs(df_logs: pd.DataFrame) -> str:
    """
    Lấy school_id đầu tiên tìm được trong cột RequestJson của log.
    Ưu tiên key 'school_id', fallback 'schoolId'.
    """
    for req in df_logs["RequestJson"]:
        try:
            data = json.loads(req)
        except Exception:
            continue

        sid = data.get("school_id") or data.get("schoolId")
        if sid:
            return sid

    raise RuntimeError("Không tìm được school_id trong RequestJson của log.")
...
def train_and_save_model(X: np.ndarray, y: np.ndarray) -> None:
...
def train_ranking_model() -> None:
    """
    Hàm public để được gọi từ API rebuild:
    - Load log
    - Load index + graph + embeddings
    - Build dataset
    - Train & save model
    """
    ...
    # 1.1. Không có data -> không train
    if n_logs == 0:
        print("⚠️ Không có log recommend, bỏ qua train model.")
        return

    # 1.2. Có biến môi trường ML_MIN_TRAIN_SAMPLES -> check số record tối thiểu
    if MIN_TRAIN_SAMPLES > 0 and n_logs < MIN_TRAIN_SAMPLES:
        print(
            f"⚠️ Chỉ có {n_logs} log (< ML_MIN_TRAIN_SAMPLES={MIN_TRAIN_SAMPLES}), "
            "bỏ qua train model."
        )
        return

    # 2. Load RAG index + graph + embeddings
    print("Detect school_id from logs...")
    try:
        school_id = get_first_school_id_from_logs(df_logs)
    except RuntimeError as ex:
        print(f"⚠ {ex} Bỏ qua train model.")
        return

    print(f"  -> Train model dùng dữ liệu của school_id = {school_id}")

    print("Loading RAG index, metadata & embeddings...")
    try:
        rag_index = RagIndex.load_for_school(school_id)
    except Exception as ex:
        print(f"⚠ Không load được RagIndex cho trường {school_id}: {ex}")
        return

    try:
        graph = IngredientGraph.load_for_school(school_id)
    except Exception as ex:
        print(f"⚠ Không load được IngredientGraph cho trường {school_id}: {ex}")
        return
    ...
    try:
        ...
        print(f"⚠️  {ex}. Bỏ qua train model.")
        return
...
def main():
    """Giữ lại main để vẫn có thể chạy file này trực tiếp."""
    train_ranking_model()
